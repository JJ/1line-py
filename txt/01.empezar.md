...
Si quieres trabajar en un entorno en el cual los paquetes que instales estan
aislados de los paquetes que estan instalados por defecto, se puede utilizar
lo que es conocido como `virtualenv`. Con `virtualenv` se puede tener entornos
donde uno puede instalar modulos que solo esta ligados a dicho entorno y no
afectan al sistema.

Para instalar se utiliza el gestor de modulos en python conocido como `pip`

```sh
$ [sudo] pip install virtualenv
```

Con esto podemos tener entornos aislados resolviendo problemas de dependencies, versiones,
y permisos. Mas información sobre virtualenv se puede encontrar [aquí](https://virtualenv.pypa.io/en/stable/)
