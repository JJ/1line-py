...
múltiplo de tres si esa cifra resultante es 3 o 9.
...
convertir en números para poder sumarlos. Una vez más uso de dos
técnicas de pensamiento computacional: buscar la representación
adecuada para los datos, y división de una operación en tres pasos (y
eso en una sola línea). 

La recursividad se usa en la siguiente, y aquí tenemos que usar la
forma tradicional de definir funciones de Python: `def`, porque con el
uso de `lambda` no se permite y en todo caso es una forma más compacta
...
anteriormente, órdenes separadas por `;`. La primera halla la suma de
las cifras, y no es estrictamente necesaria, sólo por claridad.
...
False en caso contrario. El `return` al principio de la llamada
...
Este fichero está en un lenguaje de marcas simplificado
...
HTML y con marcas simples como `#` para un titular de primer orden o
...
estrategia, pero una mejor no dabría en esa línea. El formateo se ha
...
En este caso hay que usar una palabra clave `set`, simplemente porque
ya se nos han acabado los tipos de paréntesis para usar. Ya tenemos
los corchetes, los paréntesis, las llaves... Así que se pone un `set`
y listo. 

Con estos conjuntos se puede comprobar si un elemento pertenece o no a
él:
