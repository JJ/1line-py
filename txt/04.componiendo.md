...
valors iniciales, es posible que esta estructura no sea la más
...
Aparentement es igual, pero su tipo es `tuple` en vez de `list`. Se
...
siempre, se evalúa en Clojure. Y, comos e ha indicado, esa lista no se puede modificar, como
...
tener elementos homogéneos o heterogéneos. 
